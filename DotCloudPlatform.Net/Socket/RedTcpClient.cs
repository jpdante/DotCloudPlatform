...
using DotCloudPlatform.Net.Internal;
...
public class RedTcpClient : IDisposable {
  ...
  public bool IsConnected => (_socket is { Connected: true } && _socket.Poll(1000, SelectMode.SelectRead) &&
                              (_socket.Available == 0));

  public EndPoint? RemoteEndPoint => _socket?.RemoteEndPoint;

  public RedTcpClient(string? targetHost, bool checkCertificateRevocation = true) {
    _connectDone = new ManualResetEventAsync(false);
    _targetHost = targetHost ?? null;
    _checkCertificateRevocation = checkCertificateRevocation;
    SocketStatus = SocketStatus.Disconnected;
  }

  public RedTcpClient(X509Certificate certificate, string? targetHost, bool checkCertificateRevocation = true) {
  ...
  internal RedTcpClient(System.Net.Sockets.Socket socket, string? targetHost, X509Certificate? serverCertificate, bool isEncrypted) {
    _socket = socket;
    _targetHost = targetHost ?? null;
  ...
  private async Task AuthenticateAsClient() {
    try {
      if (_targetHost == null) throw new Exception("Target host is null.");
  ...
  private async Task InitializeSocketAsClient() {
    SocketStatus = SocketStatus.Initializing;
    var buffer = new byte[] {
      0x52, 0x45, 0x44, 0x53, 0x4f, 0x43, 0x4b, 0x45, 0x54, SocketVersion.Major, SocketVersion.Minor,
      SocketVersion.Patch
    };
    ...
    int bytesRead = await _networkStream.ReadAsync(buffer.AsMemory(0, 2));
    ...
    buffer[0] = 0x06;
    await _networkStream.WriteAsync(buffer.AsMemory(0, 1));

    bytesRead = await _networkStream.ReadAsync(buffer.AsMemory(0, 1));
  ...
  internal async Task InitializeSocketAsServer() {
    ...
    int bytesRead = await _networkStream!.ReadAsync(buffer.AsMemory(0, 9));
    ...
    bytesRead = await _networkStream.ReadAsync(buffer.AsMemory(0, 3));
    ...
    if (!buffer[0].Equals(SocketVersion.Major) || !buffer[1].Equals(SocketVersion.Minor)) {
      buffer[0] = 0x15;
      buffer[1] = 0x15;
      await _networkStream.WriteAsync(buffer.AsMemory(0, 2));
      await DisconnectAsync();
